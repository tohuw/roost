...
whichever birds are running. It reports status; it does not launch anything.

## How it works

A bird publishes a small JSON **descriptor** into a shared directory saying
where it is listening and where its token lives. Roost reads whatever
descriptors it finds, fetches each bird's menu over loopback, and draws it.

That is the whole coupling. Roost has no list of known birds, no
configuration naming one, and no code that treats any particular bird
specially — a bird that is not running simply has no descriptor, and a third
bird would need no change here at all.
...
- Forwards clicks back to the bird that published them, under that bird's own
  credential — including a bird's own **Quit** or **Restart** row, which is an
  ordinary action id Roost passes back without knowing what it does
- Offers to **start** a stopped bird by asking the OS supervisor, never by
...
**Quit and Restart did.** They are not features of Roost — they are rows a bird
...
**Starting a stopped bird came along later, and carefully.** It was left out at
first on the grounds that a stopped bird has withdrawn its descriptor, so Roost
...
bird greyed out with "its recorded process is gone" and nothing to click. That
...
holding an exec path for every bird on the machine.
...
So: **the birds run themselves, the OS starts them, and Roost asks and reports.**
...
- **Declare a version range, not a version.** Roost accepts any bird whose
...
- **Publish the descriptor after binding the port, and remove it on exit.** A
  ...
  bird. A crash that skips removal is still handled — Roost checks the
