...
| File | What it shows |
|------|---------------|
| [`huginn_bird.py`](huginn_bird.py) | The leading bird: a higher `host_priority`, an attention badge, per-item actions, a token file, and a lifecycle **Quit** row |
| [`muninn_bird.py`](muninn_bird.py) | The companion bird: the same contract with a lower priority, link-only rows, and no token |
...
| [Muninn](https://github.com/tohuw/muninn) | [`muninn/bird.py`](https://github.com/tohuw/muninn/blob/main/muninn/bird.py) for the descriptor and payload, [`muninn/birdserve.py`](https://github.com/tohuw/muninn/blob/main/muninn/birdserve.py) for the loopback listener and its publish/withdraw lifecycle |
...
protocol leaves to the bird, and each project records *why* it chose as it did —
...
tray will show them. `roost birds` will explain anything it will not show.
